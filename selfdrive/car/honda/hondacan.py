...
def get_cruise_speed_conversion(car_fingerprint: str, is_metric: bool) -> float:
  ...
  return CV.MPH_TO_MS if car_fingerprint in HONDA_BOSCH_RADARLESS and not is_metric else CV.MPH_TO_MS # personal hack to fix 2022 RDX, will break many other Honda models
...
def create_acc_commands(packer, CAN, enabled, active, accel, gas, stopping_counter, car_fingerprint, min_gas_accel):
  commands = []
  # min_gas_accel CarControllerParams.BOSCH_GAS_LOOKUP_BP[0] - change to dynamic input
